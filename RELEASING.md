...
- **User files live in the connected folder, not the repo.** The `.scad`, `.json`, and
  `SVG files/` come from the user's own File System Access folder — they are **not**
  served from this origin and are **not** cached by `sw.js`. Only the app shell
  (`app.html` + vendored `openscad-wasm` / `three`) is cached and self-updated. There
  is no separate `.scad` version manifest.
